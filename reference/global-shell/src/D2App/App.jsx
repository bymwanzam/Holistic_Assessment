...
import { SessionHandler, getSessionCookie } from './components/session-handler/index.ts';
...
const Layout = ({
  ...
  const supportsSessionHandler = sessionCookie?.sessionExpiryTime && sessionCookie?.serverTime && systemInfo?.sessionTimeout;
  const appInfoQueryReady = Boolean(appsInfoQuery?.data);
  return <div className={styles.container}>
            {supportsSessionHandler && <SessionHandler sessionTimeoutInSeconds={systemInfo?.sessionTimeout} />}
            {appInfoQueryReady && <ConnectedHeaderBar appsInfoQuery={appsInfoQuery} />}
            ...
            <BrowserRouter basename={basename}>
                <Routes>
                    <Route element={<Layout appsInfoQuery={appsInfoQuery} />}>
                        <Route path="/" element={<RedirectHandler appsInfoQuery={appsInfoQuery} />} />
                        <Route path="/:appName" element={<PluginLoader appsInfoQuery={appsInfoQuery} />} />
                    </Route>
                </Routes>
            </BrowserRouter>
        </ClientPWAProvider>;
